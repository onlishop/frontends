# onlishop/frontends - api-gen

Welcome to `@onlishop/api-gen` CLI.
Generate TypeScript schemas from Onlishop OpenAPI specification.

After generating schemas, you can use them in fully typed [API Client](https://www.npmjs.com/package/@onlishop/api-client).
...
<!-- automd:pm-install name="@onlishop/api-gen" dev -->

```sh
# ✨ Auto-detect
npx nypm install -D @onlishop/api-gen

# npm
npm install -D @onlishop/api-gen

# yarn
yarn add -D @onlishop/api-gen

# pnpm
pnpm install -D @onlishop/api-gen

# bun
bun install -D @onlishop/api-gen

# deno
deno install --dev @onlishop/api-gen
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/onlishop/frontends/main/packages/api-gen/api-gen.schema.json",
...
```json
{
  "$schema": "https://raw.githubusercontent.com/onlishop/frontends/main/packages/api-gen/api-gen.schema.json",
  "patches": [
    "https://raw.githubusercontent.com/onlishop/frontends/refs/heads/main/packages/api-client/api-types/storeApiSchema.overrides.json",
...
> [!NOTE]  
> Check our current default patches to see more examples: [source](https://raw.githubusercontent.com/onlishop/frontends/main/packages/api-client/api-types/storeApiSchema.overrides.json).
...
{
  "scripts": {
    "generate-types": "onlishop-api-gen generate --apiType=store"
  }
}
```
...
```bash
pnpx @onlishop/api-gen generate --help

# generate schemas from store API
pnpx @onlishop/api-gen generate --apiType=store

# generate schemas from admin API
pnpx @onlishop/api-gen generate --apiType=admin
```
...
Load OpenAPI specification from Onlishop instance and save it to JSON file.

options:

```bash
pnpx @onlishop/api-gen loadSchema --help

# load schema from store API
pnpx @onlishop/api-gen loadSchema --apiType=store

# load schema from admin API
pnpx @onlishop/api-gen loadSchema --apiType=admin
```
...
Remember to add `.env` file in order to authenticate with Onlishop instance.

```bash
OPENAPI_JSON_URL="https://your-shop-instance.onlishop.store"
...
```bash
pnpx @onlishop/api-gen validateJson --help

# validate JSON file
pnpx @onlishop/api-gen validateJson --apiType=store
```
...
pnpx @onlishop/api-gen split <path-to-schema-file> --list tags

# Display all available paths
pnpx @onlishop/api-gen split <path-to-schema-file> --list paths

# Split schema by tags and show detailed linting errors
pnpx @onlishop/api-gen split <path-to-schema-file> --splitBy=tags --outputDir <output-directory> --verbose-linting

# Split schema by a single tag
pnpx @onlishop/api-gen split <path-to-schema-file> --splitBy=tags --outputDir <output-directory> --filterBy "media"

# Split schema by a single path
pnpx @onlishop/api-gen split <path-to-schema-file> --splitBy=paths --outputDir <output-directory> --filterBy "/api/_action/media/{mediaId}/upload"
```
...
```ts
import { generate } from "@onlishop/api-gen";
...
```ts
import { loadSchema } from "@onlishop/api-gen";
...
```ts
import { validateJson } from "@onlishop/api-gen";
...
```ts
import { split } from "@onlishop/api-gen";
...
- [📘 Documentation](https://frontends.onlishop6.com)
...
Full changelog for stable version is available [here](https://github.com/onlishop/frontends/blob/main/packages/api-gen/CHANGELOG.md)
