...
export async function resolveSinglePath<T = JSON>(
  ...
  try {
    ...
    const nodeModulesFolder = resolve(
      `node_modules/@onlishop/api-client/api-types/${pathToResolve}`,
    );
    ...
    // 4. check at github external resource for file name
    const githubFile = `https://raw.githubusercontent.com/onlishop/frontends/main/packages/api-client/api-types/${pathToResolve}`;
...
export async function loadJsonOverrides({
  ...
  const localNodePath = resolve(
    `node_modules/@onlishop/api-client/api-types/${apiType}ApiSchema.overrides.json`,
  );
  ...
  const fallbackPath = existsSync(localPath)
    ...
    : existsSync(localNodePath)
      ? localNodePath
      : `https://raw.githubusercontent.com/onlishop/frontends/main/packages/api-client/api-types/${apiType}ApiSchema.overrides.json`;
